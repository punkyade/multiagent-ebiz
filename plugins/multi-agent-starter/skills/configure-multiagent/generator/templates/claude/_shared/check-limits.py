...
측정 대상 — "워커에게 실제로 전달되는 내용"만:
  · HTML 주석 제거        — 디스패처가 brief의 HTML 주석을 **떼고 워커에 보낸다**
                            (`call_worker.sh`, 2026-08-13~). 즉 이 제외는 임의 규칙이 아니라
                            실제 전달량과의 일치다. 그 전에는 주석으로 옮기는 것이
                            "측정상 절감"에 불과하다는 지적이 성립했다(codex-critic).
