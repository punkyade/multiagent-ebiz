...
## 첫 실전 작업 1건이 하네스 결함 3건을 드러냈다 — 도구를 더 만드는 것보다 한 바퀴 돌리는 게 먼저 (2026-08-13, mat-purpose-fix)
- **배경**: 승인 감사·한도 검사·환경 진단·스캐폴더·프리셋을 만들었지만 `tasks/`가 비어 있어 **실전 검증 0**이었다. 파일럿으로 KI-1 수정을 하네스로 수행했다(claude-main 설계 → codex-critic 검증 → Orchestrator 적용).
- **결함 ① 디스패처가 유료 호출 결과를 통째로 유실**: codex-critic이 1m27s 정상 수행했는데 최종 envelope를 `jq --argjson e "$env"` 로 **argv에** 실어 넘기다 `Argument list too long`(exit 126)으로 사망. 워커 출력이 argv 한계(Windows ~32KB)를 넘으면 **항상** 터진다. 수정: herestring(stdin) 전달. 회귀 가드로 198,893자 보존 실측 고정. → **단위 테스트로는 절대 안 나온다.** 큰 출력을 내는 실제 워커를 불러야 나온다.
- **결함 ② brief 범위 오류**: 오케스트레이터(나)가 대상을 "4벌"로 적었으나 실제 16벌(루트 4 + 3 flavor × 4). claude-main이 잡았다. **워커가 오케스트레이터의 전제를 교정한 사례** — brief에 "대상 목록"을 적을 땐 워커에게 목록 자체의 완전성도 검증시키는 게 낫다.
- **결함 ③ 측정 지표가 실제와 어긋남**: `check-limits`가 HTML 주석을 한도에서 제외하는데, 디스패처는 `@brief_content = cat` 으로 **주석 포함 전문**을 워커에 보내고 있었다. codex-critic이 "주석에 임의 내용을 숨겨 한도를 무력화할 수 있다"고 반박. 수정: 디스패처가 brief의 주석을 떼고 전달 → 측정과 실제가 일치. → **지표를 만들 땐 "이 숫자가 무엇을 대리하는가"와 "실제 그 값과 같은가"를 분리해 확인해야 한다.**
- **Producer-Reviewer가 실제로 작동했다**: ③은 설계자(claude-main)가 스스로 Caveat E로 "회피로 볼 여지"까지 적었지만 판단은 유보했고, **독립 리뷰어가 반박해야** 결론이 났다. 단독 워커였으면 그대로 배포됐다.
- **일반 교훈**: ① 도구를 쌓는 동안 쌓인 것은 **미검증 표면**이다. 실전 1건이 그 표면을 한 번에 훑는다. ② 유료 호출은 **결과 수신 경로까지** 검증 대상이다 — 호출이 성공해도 조립 단계에서 잃으면 비용만 나간다. ③ 리뷰어에게 "동의가 아니라 반례를 찾으라"고 명시하면 실제로 반례가 나온다.
**worker**: claude-main(설계·범위교정) ∥ codex-critic(반박) → orchestrator(적용·검증)
