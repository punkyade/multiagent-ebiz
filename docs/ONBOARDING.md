...
실행 — **터미널을 하나 더 열고**, 프로젝트 폴더에서 `mat` 한 줄이면 됩니다:
...
작업 이름을 안 줘도 **지금 진행 중인 작업을 자동으로 고릅니다.** 고르는 순서는:

1. 인자로 준 작업 (`mat <작업명>`)
2. `status`가 `in_progress` · `reviewing` · `waiting_*` 인 것 중 **가장 최근 수정된 것**
3. 없으면 작업 목록 모달 (`t`와 동일)

> 즉 **`status`를 성실히 갱신하면 `mat` 한 글자로 지금 도는 작업이 바로 뜹니다.**
> 끝난 작업(`done`)은 2번에 안 걸려 목록 모달이 뜨는데, 이건 정상입니다.

다른 폴더에서 띄우려면 `MAT_ROOT`로 지정합니다:

```bash
MAT_ROOT=<프로젝트 폴더> mat
```
...
**2초마다 자동 갱신**됩니다. 워커가 도는 동안 띄워두면 진행이 실시간으로 보입니다.
...
| `L` / `l` | 로그 모달 (`j`/`k` 스크롤 · `g`/`G` 처음·끝) |
| `u` | 사용량 — **`coach` 필요. 미설치면 빈 화면이 정상** (아래 5-2를 쓰세요) |
| `q` | 종료 |
